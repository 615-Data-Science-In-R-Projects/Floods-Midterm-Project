...
f[1:12,] |> kbl(caption = "Table 1: Floods in 2020 & 2021") %>%
...
### Poverty across States and Flooded Areas
...
ggplot(comparisons_state_groups, aes(State,Deaths))+
  geom_point(size=1, alpha=0.9)+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  labs(title="Are there more deaths in states where floods \naffect areas richer than the state average",
       y= "Deaths")+
  facet_wrap(~poorer_areas_flooded)
...
The deaths from floods in 2020-2021 were zero in most states which is great news. However it appear Tennessee really suffered, with close to 20 deaths from one flood alone. 

By far the most interesting graph of this trifecta is the one detailing property damage.It makes sense that Florida, with coastal flooding might have had more property damage, being more densely populated. West Virginia had a lot of floods but had lower property damage levels than Florida's two floods and its mean property damage is the lowest of all these states. Tennessee, West Virginia and Kentucky are the most interesting of this bunch. All three have had a number of floods which span a large range of property damage values. Could this be perhaps because the floods have occured across both rural and urban areas? Or is it perhpas