...
census_poverty_data<- census_poverty_data |> 
              mutate(County = 
        str_trim(str_sub(`Geographic Area Name`,1,
...
census_poverty_data<- census_poverty_data |> mutate(
  State=str_trim(str_sub(`Geographic Area Name`,
...
```
We have coastal states like New York and Florida represented but it would appear that the large majority of floods happen in non-coastal states. 
...
Let's start by visualizing how poverty varies across the states in question. We can limit ourselves to the top ten most flooded states as indicated above. 

```{r}
#| echo: false
state_list<- c("Kentucky","South Dakota", "Tennessee", "Arkansas", "Missouri", 
               "Illinois", "Iowa","New York", "Indiana", "Florida")
...
ggplot(state_poverty_level, aes(State, `Below poverty level`))+
  ...
  annotate("text", x="Kentucky",
           y=11.8, label="US Average percent below poverty level = 11.5%")+
